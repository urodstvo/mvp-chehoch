...
def get_organisation(organisation_id: int):
    try: 
        ...
        org = Organisation(
            ...
            address=Organisation.string_value_to_str(response.organisation.adress),
            ...
            logo=Organisation.int64_value_to_int(response.organisation.logo),
           
        )

        if org.logo:
            file = SurveyServiceClient.GetFile(survey_pb2.GetFileRequest(file_id=org.logo))
            org.logo_url =  f"{MINIO_URL}/chehoch/{file.file.filename}"
